  ...
  static void replicateUpdateRecordTest(MockCluster cluster, SSLConfig clientSSLConfig, Properties routerProps,
    ...
    try {
      ...
      // verify Blob(compactedBlobId) really got compacted on the source host.
      {
        ...
        int correlationId = 1;
        AdminRequest adminRequest = new AdminRequest(AdminRequestOrResponseType.BlobIndex, null, correlationId, "clientId1");
        BlobIndexAdminRequest blobIndexRequest = new BlobIndexAdminRequest(compactedBlobId, adminRequest);

        stream = sourceChannel.sendAndReceive(blobIndexRequest).getInputStream();
        ...
        assertEquals(1, messages.size());
        MessageInfo deleteInfo = messages.values().stream().findFirst().get();
        assertEquals(true, deleteInfo.isDeleted());
      }
      ...
      // Even we read with GetOption.Include_All, it returns Blob_Deleted
      {
        ...
        stream = targetChannel.sendAndReceive(blobIndexRequest).getInputStream();
        AdminResponseWithContent adminResponse = AdminResponseWithContent.readFrom(stream);
        assertEquals(ServerErrorCode.No_Error, adminResponse.getError());
